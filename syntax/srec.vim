...
syn match srec16BitAddress "^S[0159][0-9a-fA-F]\{6}" contains=srecLength
...
syn match srecVersion "^S0[0-9a-fA-F]\{8}" contains=srec16BitAddress
syn match srecRevision "^S0[0-9a-fA-F]\{10}" contains=srecVersion
...
if version >= 508 || !exists("did_srec_syntax_inits")
  ...
  HiLink srecChecksum		PreProc
  HiLink srecVersion		Include
  HiLink srecRevision		Number
